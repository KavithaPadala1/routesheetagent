...
async def format_corrosionroutesheet_results(user_query: str, sql_results: list) -> str:
    """
    ...
    try:
        ...
        formatting_prompt = f"""
You are a response formatter for a Corrosion Routesheet Insights AI assistant.
...
- At the end of the response , suggest next question the user can ask related to corrosion routesheet data to encourage further engagement when appropriate. Keep the only one suggestes question.
...
"""
        ...
        print(f"[corrosionroutesheet_formatter] Response formatted successfully")
        
        return formatted_response
        
    except Exception as e:
        error_message = f"Error formatting results: {str(e)}"
        print(f"[corrosionroutesheet_formatter] {error_message}")
        import traceback
        traceback.print_exc()
