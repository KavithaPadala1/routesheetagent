# leaksurveyroutesheet_formatter.py - Formats SQL query results for leak survey routesheet agent
...
async def format_leaksurveyroutesheet_results(user_query: str, sql_results: list) -> str:
    """
    ...
    try:
        ...
        formatting_prompt = f"""
You are a response formatter for a Leak Survey Routesheet Insights AI assistant.
...
- At the end of the response, suggest next question the user can ask related to leak survey data to encourage further engagement when appropriate.
...
"""
        ...
        print(f"[leaksurveyroutesheet_formatter] Response formatted successfully")
        
        return formatted_response
        
    except Exception as e:
        error_message = f"Error formatting results: {str(e)}"
        print(f"[leaksurveyroutesheet_formatter] {error_message}")
        import traceback
        traceback.print_exc()
