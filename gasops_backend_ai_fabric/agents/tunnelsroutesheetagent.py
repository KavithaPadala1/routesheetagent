...
# tunnelsroutesheet.py - Handles  tunnels routesheet-related queries by generating and executing SQL

import json
from datetime import datetime
from zoneinfo import ZoneInfo
from config.azure_client import get_azure_chat_openai
from prompts.tunnelsroutesheetprompt import get_tunnelsroutesheet_sql_prompt
from tools.sql_executor import execute_sql_query_with_retry, get_sql_tool_definition

from tools.tunnelsroutesheet_formatter import format_tunnelsroutesheet_results
from aisearch.ai_search import tunnels_routesheet_search  # Import AI Search


async def handle_tunnelsroutesheet(query: str, auth_token: str = None):
    """
    ...
    try:
        ...
        print(f"[tunnelsroutesheetagent] Processing query: {query}")
        print(f"[tunnelsroutesheetagent] Current year: {current_year}, Current date: {current_date}")

        # Fetch relevant examples from AI Search
        print("[tunnelsroutesheetagent] Fetching relevant examples from AI Search...")
        search_results = tunnels_routesheet_search(query)
        ...
        if search_results:
            ...
            for idx, result in enumerate(search_results[:3], 1):  # Top 3 results
                examples_context += f"\nExample {idx}:\n{result.page_content}\n"
            print(f"[tunnelsroutesheetagent] Found {len(search_results)} relevant examples")
            print(f"[tunnelsroutesheetagent] Examples context:\n{examples_context}")
        else:
            print("[tunnelsroutesheetagent] No relevant examples found in AI Search")


        # Get SQL generation prompt (simplified - no formatting rules)
        system_prompt = get_tunnelsroutesheet_sql_prompt(query, current_year)  
        ...
        print("[tunnelsroutesheetagent] Calling LLM to generate SQL...")
        ...
        if not tool_calls:
            ...
            print(f"[tunnelsroutesheetagent] No tool call. Direct response from LLM.")
        ...
        for tool_call in tool_calls:
            ...
            if function_name == "execute_sql_query":
                ...
                try:
                    ...
                    print(f"[tunnelsroutesheetagent] Query returned {len(sql_results) if sql_results else 0} rows")

                    # Format results using dedicated formatter
                    print("[tunnelsroutesheetagent] Calling formatter to create user response...")
                    formatted_answer = await format_tunnelsroutesheet_results(query, sql_results)
                    ...
                    print(f"[tunnelsroutesheetagent] {error_msg}")
        ...
        error_message = f"Error in tunnelsroutesheetagent: {str(e)}"
        print(f"[tunnelsroutesheetagent] {error_message}")
        import traceback
        traceback.print_exc()
