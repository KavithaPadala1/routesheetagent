...
# corrosionroutesheetagent.py - Handles corrosion routesheet-related queries by generating and executing SQL

import json
from datetime import datetime
from zoneinfo import ZoneInfo
from config.azure_client import get_azure_chat_openai
from prompts.corrosionroutesheetprompt import get_corrosionroutesheet_sql_prompt
from tools.sql_executor import execute_sql_query_with_retry, get_sql_tool_definition

from tools.corrosionroutesheet_formatter import format_corrosionroutesheet_results
from aisearch.ai_search import corrosion_routesheet_search  # Import AI Search


async def handle_corrosionroutesheet(query: str, auth_token: str = None):
    """
    ...
    try:
        ...
        print(f"[corrosionroutesheetagent] Processing query: {query}")
        print(f"[corrosionroutesheetagent] Current year: {current_year}, Current date: {current_date}")

        # Fetch relevant examples from AI Search
        print("[corrosionroutesheetagent] Fetching relevant examples from AI Search...")
        search_results = corrosion_routesheet_search(query)
        ...
        if search_results:
            ...
            for idx, result in enumerate(search_results[:3], 1):  # Top 3 results
                examples_context += f"\nExample {idx}:\n{result.page_content}\n"
            print(f"[corrosionroutesheetagent] Found {len(search_results)} relevant examples")
            print(f"[corrosionroutesheetagent] Examples context:\n{examples_context}")
        else:
            print("[corrosionroutesheetagent] No relevant examples found in AI Search")


        # Get SQL generation prompt (simplified - no formatting rules)
        system_prompt = get_corrosionroutesheet_sql_prompt(query, current_year)  
        ...
        print("[corrosionroutesheetagent] Calling LLM to generate SQL...")
        ...
        if not tool_calls:
            ...
            print(f"[corrosionroutesheetagent] No tool call. Direct response from LLM.")
        ...
        for tool_call in tool_calls:
            ...
            if function_name == "execute_sql_query":
                ...
                try:
                    ...
                    print(f"[corrosionroutesheetagent] Query returned {len(sql_results) if sql_results else 0} rows")

                    # Format results using dedicated formatter
                    print("[corrosionroutesheetagent] Calling formatter to create user response...")
                    formatted_answer = await format_corrosionroutesheet_results(query, sql_results)
                    ...
                    print(f"[corrosionroutesheetagent] {error_msg}")
        ...
        error_message = f"Error in corrosionroutesheetagent: {str(e)}"
        print(f"[corrosionroutesheetagent] {error_message}")
        import traceback
        traceback.print_exc()
