...
# Leak Survey Route Sheet SQL Query Generator Prompt 
...
SCHEMA_PATH = os.path.join(os.path.dirname(__file__), "..", "schema", "leaksurveyroutesheet_schema.txt")
...
def get_leaksurveyroutesheet_sql_prompt(user_query: str, current_year: int, ai_search_examples: str = ""):
    """
    Generate the leak survey routesheet agent prompt for SQL generation ONLY.
...
"""
    ...
    print(f"[leaksurveyroutesheetprompt] Current date: {current_date}, Current year: {current_year}, Current day: {current_day}")

    
    return f"""
You are an expert SQL query generator for Microsoft Fabric Warehouse, specialized in Leak Survey Route Sheet related data.
...
## TABLE USAGE GUIDELINES FOR LEAK SURVEY ROUTE SHEET QUERIES:

a) cedemo_ContractorsLeakSurveyEmployeeScheduleDetails - Contractor leak survey employee schedules
   - Use for contractor employee schedule details including planned assignments, daily shifts, qualifications, notes
   - Contains daily columns for Sunday through Saturday (dates, shifts, notes, qualified work descriptions, etc.)
   
b) CEDEMO_CompanyLeakSurveyEmployeeScheduleDetails - Company leak survey employee schedules
   - Use for company employee schedule details including planned assignments, daily shifts, qualifications, notes
   - Contains daily columns for Sunday through Saturday (dates, shifts, notes, qualified work descriptions, etc.)

## when user didn't specify contractor or company , always default to company leak survey routesheet.
...
When user asks for "summary" or "tell me about leak survey routesheet", generate queries for relevant sections:
- Employee counts by region/week
- Shift assignments
- OQ exceptions
- DISA pool exceptions
- Notes and special assignments
- Vehicle assignments

**Note:** Adapt your queries based on whether the user is asking about contractor or company leak survey data.
"""