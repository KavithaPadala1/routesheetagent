# GDS STEADY Route Sheet SQL Query Generator Prompt
...
SCHEMA_PATH = os.path.join(os.path.dirname(__file__), "..", "schema", "gdssteadyroutesheet_schema.txt")
...
def get_gds_steady_sql_prompt(user_query: str, current_year: int, ai_search_examples: str = ""):
    """
    Generate the GDS STEADY routesheet agent prompt for SQL generation ONLY.
...
"""
    ...
    print(f"[gds-steady-prompt] Current date: {current_date}, Current year: {current_year}, Current day: {current_day}")

    return f"""
You are an expert SQL query generator for Microsoft Fabric Warehouse, specialized in GDS/ERF STEADY Route Sheet data.
...
3. Always apply WHERE IsActive = 1 (not IsDeleted for steady tables).
...
## TABLE USAGE GUIDELINES FOR GDS STEADY ROUTE SHEET QUERIES:

a) CEDEMO_MechASteadyEmployeeScheduleDetails - Mechanical A steady employee schedules
   - Use for Mech A steady employee schedule details including assignments, daily shifts, area details, qualifications, notes
...
b) CEDEMO_MechBSteadyEmployeeScheduleDetails - Mechanical B steady employee schedules
   - Use for Mech B steady employee schedule details including assignments, daily shifts, area details, qualifications, notes
   - Contains daily columns for Sunday through Saturday (dates, shifts, area details, notes, qualified/not qualified work descriptions, missing covered task numbers, DISA pool, etc.)

c) CEDEMO_ERFEmployeeScheduleDetails - ERF employee schedules (can be steady or rotating)
   - Use for ERF steady employee schedule details when query mentions ERF steady
...
## OQ Exceptions (for Mech A and Mech B):
...
## DISA Pool Exceptions (for Mech A and Mech B):
...
When user asks for "summary" or "tell me about GDS steady routesheet", generate queries for relevant sections:
- Overall details like shift details, region, employees, area details
- OQ exceptions (for Mech A/B)
- DISA pool exceptions (for Mech A/B)

**Note:** Adapt your queries based on whether the user is asking about Mech A, Mech B, or ERF steady data.
"""